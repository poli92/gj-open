...
# Question: 
Will ACLED record any civilian fatalities in Angola in October 2017?
...
df <- read.csv("X:\\Personal Development\\Data Science\\HFC\\angola_fatalities\\ACLED-Version-7-All-Africa-1997-2016_csv_dyadic-file.csv", header=FALSE, stringsAsFactors = FALSE)
...
Subset to relevant Angola data and convert EVENT_TYPE to character
```{r}
ang_data <- df[which(df$COUNTRY == 'Angola' & (df$EVENT_TYPE %in% c('Violence against civilians', 'Remote Violence'))), ]
```

Create a date variable consistent with a normal R date 
```{r}
ang_data$DATE <- as.Date(ang_data$EVENT_DATE, format = "%d/%m/%Y")
```
...
```{r}
daily_data <- merge(x = full_range, y = ang_data, by = "DATE", all.x = TRUE)
...
Create a corpus
```{r}
angCorpus <- Corpus(VectorSource(ang_data$NOTES))
```

Remove punctuation and stopwords from corpus
```{r}
angCorpus <- tm_map(angCorpus, removePunctuation)
angCorpus <- tm_map(angCorpus, removeWords, stopwords('english'))
```

Perform stemming - words are converted to their stem so as not to duplicate root words that are used in different forms
```{r}
angCorpus <- tm_map(angCorpus, stemDocument)
```

Plot the word cloud
```{r}
wordcloud(angCorpus, max.words = 100, random.order = FALSE, colors=brewer.pal(8, "Dark2"))
```
